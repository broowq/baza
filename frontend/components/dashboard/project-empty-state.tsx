    ...
    <div className="empty-state panel-glass elev-2">
      ...
      <p className="empty-state__body">
        {canManage ? (
          ...
          <>
            В проекте пока нет лидов. Запустить сбор может владелец или администратор —
            как только он это сделает, компании{niche ? ` по нише «${niche}»` : ""}
            {geography ? ` — ${geography}` : ""} появятся здесь автоматически.
          </>
        )}
      </p>
      {canManage && (
