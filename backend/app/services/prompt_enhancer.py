...
Правильно: 01.47 (птицеводство), 01.46 (свиноводство) — это покупатели кормов."""
    ...
    try:
        ...
        wp = wp if wp in ("any", "no_website", "with_website") else "any"
        # БЭКСТОП (E2E 17.07): LLM часто игнорирует инструкцию и возвращает
        # "any" при явном «нет своего сайта» — ровно инцидент 14.07. Regex-
        # эвристика literal и надёжна; если она уверена, а LLM сказал "any" —
        # доверяем эвристике. LLM-вердикт (не-"any") сохраняем: он ловит
        # формулировки, которые regex пропускает.
        if wp == "any":
            wp = _website_preference_heuristic(raw_prompt)
        result["website_preference"] = wp
...
def _website_preference_heuristic(raw_prompt: str) -> str:
    ...
    # «у нас/у меня нет (своего) сайта» — про САМОГО пользователя, не про
    # клиентов (ревью 14.07: без выреза проект терял весь веб-проход).
    t = re.sub(r"у\s+(нас|меня)\s+(пока\s+)?нет\s+(?:\w+\s+){0,2}сайт\w*", " ", t)
    # «нет … сайт» с прилагательным между («нет своего/собственного сайта»):
    # до 2 слов зазора (E2E 17.07: «нет своего сайта» не ловилось вплотную).
    # Границы слов обязательны: «интерНЕТ сайты» не должен ловиться как «нет».
    if re.search(r"\b(нет|без|отсутству\w*)\s+(?:\w+\s+){0,2}сайт", t):
        return "no_website"
