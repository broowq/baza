...
class Settings(BaseSettings):
    ...
    # ── Анти-брутфорс логина (см. services/login_guard.py) ──────────────
    # Порог неудачных попыток входа НА АККАУНТ, после которого — кулдаун.
    # Считается по нормализованному email (не по IP): защищает от подбора
    # пароля к одному аккаунту с меняющихся IP. В dev/тестах отключён.
    login_max_failed_attempts: int = 5
    # Длительность кулдауна (мин) после превышения порога. Он же — окно, за
    # которое копятся неудачные попытки (TTL счётчика). 15 мин = ~20 попыток
    # в час на аккаунт при полном переборе, что убивает онлайн-брутфорс, но
    # разблокирует честного пользователя после короткой паузы.
